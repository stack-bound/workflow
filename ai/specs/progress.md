...
| Stage 3 — AI agents | 🚧 In progress (status-visibility slice) | 2026-06-15 | — |
...
- [x] `internal/status` — per-workspace status files; hook installation (`wf hooks install`) — landed ahead of the runner so Claude Code / any agent can drive the icons today
- [x] Sidebar/dashboard show agent status (working / waiting / idle), live (fsnotify) + tmux tab icons
...
- **2026-06-15** — **Sidebar/dashboard status transport resolved** (build-plan open question). The dashboard uses **fsnotify** on the status dir for instant updates (4s git tick kept as a safety net); the sidebar keeps its ~1.5s poll. Status lives in per-workspace files under `<config>/status/` (not the registry `status_slot`, which stays unused) — high-frequency writes on every tool call would churn the registry under flock. Also: the status-visibility slice of Stage 3 landed ahead of the agent runner, so the icons work with Claude Code (or any agent) via `wf hooks install` now.
...
## Changelog
- **2026-06-29** — **Hooks for all tabs (`feature/hooks-for-all`).** Agent status now decorates **every** tmux tab, not just wf-opened worktree windows. `set-status` stopped resolving a window by the `@wf_workspace` tag and instead **always decorates the current window** (the pane the hook fired in, via `$TMUX_PANE`); the tag is now only the **owned-vs-borrowed discriminator**. Owned windows (worktree or project base) are decorated permanently (strip-and-prepend the glyph on the existing name, no git call; idle glyph kept on teardown); **borrowed** windows (an agent started by hand anywhere) are **adopted-and-reverted** — the first touch snapshots the original name + `automatic-rename` into `@wf_prev_*` markers (idempotent, so a re-used stale tab keeps its true original) and pins `automatic-rename` off, and `done` fully reverts (name + `automatic-rename` restored, markers and tab style cleared), so wf stays a polite guest. New **`ready` ("your turn")** state (green bell, distinct from idle "no session"): the hook map is now `Stop`→`ready` (end of every turn) and a **new `SessionEnd`→`done`** (teardown), so a finished-but-parked agent is no longer indistinguishable from no agent. `Effective()` is **per-state**: only `working` ages out (it self-refreshes on every `PostToolUse`); `waiting`/`ready` **persist** until the next state change or `SessionEnd` — fixing the "stuck on a 30-minute call hides a waiting agent" bug — and the default TTL rose **5m → 30m** (configurable, `≤0` disables). The dashboard **base/root row now shows agent status**: `set-status` writes a base status file keyed by the project root with an empty branch component (`status.WriteBase`/`ReadBase`, mirrored on the read side via a new `ResolveProjectByCwd`), `readStatuses` reads each project's `Main`, and `renderMainRow` renders the agent cell (was a hardcoded blank); `ready` joins the legend and rows. New **`status.scope: all|wf`** config (default `all` — decorate any current window; `wf` — only registered worktrees/bases, leaving unrelated tabs untouched), and `cwd` is read from the hook's **stdin JSON** (falling back to `os.Getwd`). Cleanup leans on `SessionEnd` + self-heal-on-reuse; the residual orphan is swept by a new **`wf status reset`** (revert all windows still carrying the markers, server-wide) — no polling janitor. **Upgrade path:** opening the dashboard **drift-corrects an already-installed user's hooks** (adds `SessionEnd`, flips `Stop`→`ready`) with a one-line notice, guarded to never install from absence or undo a `hooks uninstall`, and judged on the reported state (not the embedded binary path, so a different invocation path never churns the file); there is no launch-time prompt and hooks carry no version stamp. Tests across all new units (per-state `Effective`, `ResolveProjectByCwd`, base-key round-trip, `StripGlyph` incl. the ascii edge, `AutoRenameSnapshot`/adopt/revert on an isolated server, owned/borrowed decoration + cwd-from-stdin + base-file + scope gating + `status reset` sweep, base-row agent cell + `ready` legend) and verified end-to-end on an isolated tmux server (`-L`): borrowed adopt→working/ready→full revert, base-row light-up + status file, 30m persistence, `status reset`, scope gating — default server untouched. Total coverage held (80.5% → 81.2%).
- **2026-06-23** — **Dashboard relayout + base-checkout rows.** Every project now renders a **launchable base-checkout row** (the repo root): it shows the branch the root is on and its clean/dirty state, and the same keys that drive worktrees act on the trunk — `t` opens a tmux window on the base branch at the project root (tagged with `@wf_workspace` like any workspace, so the `▣` indicator and `resurrect` work for it too), `e`/`o` open it in an editor, `enter` shows the root's uncommitted diff (`git diff HEAD`), so the base can be launched without first carving a worktree. Engine: `ProjectView.Main` (`workspace.MainCheckout{Path,Branch,Dirty,Err}`) derived in `Ledger()`, plus `Manager.MainDiff()` and `git.Dirty()`. The whole dashboard was restyled into a polished lipgloss surface: a full-width header bar (app name + project/worktree summary), a Catppuccin Mocha theme (hex, not ANSI indices), one top column header, project→worktree **tree connectors** (`├─`/`└─`), aligned status columns, a highlighted selected row, a full-width footer help bar, and a dimmed, home-shortened (`~`), left-truncated path on every project and worktree row. Verified end-to-end on an isolated tmux server (`-L wf_test`): layout/alignment, base-branch launch + window tagging, base-checkout diff, selection highlight, and truecolor rendering. Tests cover the new engine (`MainCheckout`/`MainDiff`/`git.Dirty`), the base-row launch/diff/copy/edit handlers, and the pure render helpers (state/summary/connectors/path-shortening/truncation); total coverage held (79.2%).
- **2026-06-15** — **Live agent-status icons landed (Stage 3 slice).** A new `internal/status` package is the single source of truth: a per-workspace file under `<config>/status/<project>__<branch>-<hash>.json` (`{state, ts, branch, project}`), written by a new hidden `wf set-status <state>` command that infers the workspace from cwd and no-ops (exit 0) outside one — safe to fire on every tool call. `wf hooks install`/`uninstall`/`print` merge the four Claude Code hooks (UserPromptSubmit/PostToolUse→working, Notification→waiting, Stop→done) into `~/.claude/settings.json` idempotently, preserving the user's other hooks. tmux: every wf-opened window carries a status glyph **inside the tab** (prefix on the window name, `<glyph> <branch>`), with the whole tab recolored per state (`color_mode: tab|glyph|none`, default tab) and reverting to the user's theme on idle. Dashboard: a status glyph column updates **live via fsnotify** (4s git tick kept as a safety net), highlighting working/waiting workspaces; the sidebar's agent-status slot is filled too. Glyphs/colors are configurable (`status:` block; presets `nerdfont|emoji|ascii`) and a TTL (default 5m) self-heals a stuck "working" when an agent dies without a Stop hook. `rm`/`merge` delete the status file; `close` resets it to idle. Tests across all new units (status keying/IO/TTL/resolve, config resolver, tmux name/style builders + isolated-server appliers, set-status incl. tmux tab update, hooks merge/idempotency/non-clobber, dashboard fsnotify + render, sidebar slot, and rm/merge/close lifecycle) — total coverage held (≈80%).
